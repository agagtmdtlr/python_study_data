...
# x1, x2, y
data = [[2.0, 0, 81], [4, 4, 93], [6, 2, 91], [8, 3, 97]]
...
mydic = {x1:x1_data, x2:x2_data, y:y_data}
epochs = 100000+1
for step in range(epochs) :
    sess.run(train, feed_dict=mydic)
    if (step % 5000 == 0) :
        # 만번을 반복하되 1000번 단위로 출력하여 보겟다 ( 디테일 하게 볼꺼면 1000>10 이런식으로 줄이면 됨)
        _cost, _w1, _w2, _b, _H = sess.run([cost, w1, w2, b, H], feed_dict=mydic)
        print('epoch : %d' % (step))
        print('_cost : %f' % (_cost))
        print('_w : %d' % (_w1))
        print('_w : %d' % (_w2))
...
test_dict = {x1:x1_test, x2:x2_test}
